 ...
 */
@property (nonatomic, strong) id<WFBikePowerDelegate> cpmDelegate;

/**
 * Specifies the WFBikeTrainerDelegate instance which will handle trainer callback
 * methods for this WFBikePowerConnection.
 */
@property (nonatomic, strong) id<WFBikeTrainerDelegate> btDelegate;
...
/**
 * Sends request to enable or disable the specified bike trainer feature
 *
 * @param eFeature The bike trainer feature to enable or disable
 * @param bEnable Set <c>TRUE</c> if the specified feature should be enabled, 
 *                <c>FALSE</c> if the specified feature should be disabled.
 *
 * @return <c>TRUE</c> if the command was sent successfully, otherwise <c>FALSE</c>.
 */
- (BOOL)trainerSetFeatureEnabledState:(WFBikeTrainerFeature_t)eFeature enable:(BOOL)bEnable;


/**
 * Sends request to read the enabled/disabled state for the specified bike trainer feature
 *
 * @param eFeature The bike trainer feature to get enabled/disabled state
 *
 * @return <c>TRUE</c> if the command was sent successfully, otherwise <c>FALSE</c>.
 */
- (BOOL)trainerReadFeatureEnabledState:(WFBikeTrainerFeature_t)eFeature;


/**
 * Sends request to set the device id for an ANT device to be paired
 *
 * @param eChannelType The channel type for the ANT device
 * @param usDeviceID The device id for the ANT device
 *
 * @return <c>TRUE</c> if the command was sent successfully, otherwise <c>FALSE</c>.
 */
- (BOOL)trainerSetPairedAntDeviceId:(WFBikeTrainerPairedAntChannelType_t)eChannelType deviceId:(USHORT)usDeviceId;


/**
 * Sends request to read the device id for the specified channel type
 *
 * @param eChannelType The channel type for the ANT device
 *
 * @return <c>TRUE</c> if the command was sent successfully, otherwise <c>FALSE</c>.
 */
 - (BOOL)trainerReadPairedAntDeviceId:(WFBikeTrainerPairedAntChannelType_t)eChannelType;


/**
 * Sends request to read the connection state of the paired ant device
 *
 * @param eChannelType The channel type for the ANT device
 * @param usDeviceID The device id for the ANT device
 *
 * @return <c>TRUE</c> if the command was sent successfully, otherwise <c>FALSE</c>.
 */
 - (BOOL)trainerReadPairedAntDeviceState:(WFBikeTrainerPairedAntChannelType_t)eChannelType deviceId:(USHORT)usDeviceId;
 ...
 * @return <c>TRUE</c> if the command was sent successfully, otherwise <c>FALSE</c>.
 */
- (BOOL)trainerRequestAntConnection:(WFSensorType_t)eSensorType deviceId:(USHORT)usDeviceId useForPower:(BOOL)bUseForPower __attribute__((unavailable("Use sendSetPairedAntDeviceId:deviceId:")));


/**
 * Sends request to get the supported and currently required calibrations.
 * It is recommended that the required calibrations are performed to ensure
 * proper functioning of the device.
 *
 * @return <c>TRUE</c> if the command was sent successfully, otherwise <c>FALSE</c>.
 */
- (BOOL)trainerReadCalibrationCapabilities;
