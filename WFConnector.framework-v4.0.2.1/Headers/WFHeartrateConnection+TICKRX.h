...
/**
 * The callback for the resetActivityCalibrationToDefaultsForActivityType: control point method.
 *
 * @param error If an error occurs, an error object containing the error information will be provided.  The error is limited to WFDeviceControlPointErrorDomain.
 */
- (void) hrTICKRXConnection:(WFHeartrateConnection*)connection didResetActivityCalibrationToDefaultsWithError:(NSError*)error;
...
/**
 * @return The current running speed (m/s). <br>  Important: for best performance do not return the running speed from TICKRX
...
/**
 * Reset the calibration to default for a specific activity type.  The delegate will be notified using the appropriate callback.
 * @see WFHeartrateConnectionTICKRXDelegate hrTICKRXConnection:didResetActivityCalibrationToDefaultsWithError:
 *
 * @param activityType The activity type for which the calibration should be reset.
 *
 * @return <c>FALSE</c> if the command could not be sent.<br>Note: a return value of <c>TRUE</c> does not guarantee that the command completed successfully.
 */
...
#pragma mark - Methods related to device vibration
/**@name Methods related to device vibration */
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/**@{*/

/** Perform vibration
 * @param vibrationPattern The vibration timing pattern, consisting of an array with up to five vibration step objects.
 *        Each step object is a dictionary containing vibration on- and off duration in seconds: @{@"onDuration": @(<on duration in seconds>), @"offDuration": @(<off duration in seconds>)}
 */
