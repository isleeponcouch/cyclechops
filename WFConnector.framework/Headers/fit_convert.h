...
typedef struct
{
	...
	FIT_UINT8                dev_data[100]; ///< developer data field buffer
#if defined(FIT_CONVERT_TIME_RECORD)
...
FIT_CONVERT_RETURN FitConvert_Read(const void* data,
								   FIT_UINT32 size);
...
FIT_CONVERT_RETURN FitConvert_ReadExt(const void* data,
									  FIT_UINT32 size,
...
#if defined(FIT_CONVERT_MULTI_THREAD)
FIT_UINT32 WFFitConvert_CurrentOffset(FIT_CONVERT_STATE* state);
#else

FIT_UINT32 WFFitConvert_CurrentOffset(void);

#endif

#if defined(FIT_CONVERT_MULTI_THREAD)
const FIT_UINT8 *FitConvert_GetDevFieldData(FIT_CONVERT_STATE *state);
#else
const FIT_UINT8* FitConvert_GetDevFieldData(void);
#endif
